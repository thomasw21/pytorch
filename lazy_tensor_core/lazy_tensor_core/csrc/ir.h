...
class Node {
 public:
  ...
  // TODO(whc) any reason to have a default value for hashes at Node base? I did
  // it now just to avoid having to reorder ctor args, but might do that later.
  Node(OpKind op, OpList operands, size_t num_outputs = 1,
       lazy_tensors::hash_t node_hash = kHashSeed,
       lazy_tensors::hash_t dag_hash = kHashSeed);
